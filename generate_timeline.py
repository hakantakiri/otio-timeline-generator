...
def display_ratio(
    ...
    reported_display_ratio = parse_ratio(stream.get("display_aspect_ratio"))
    if reported_display_ratio is not None:
        if rotation_degrees is not None and abs(round(rotation_degrees)) % 180 == 90:
            return 1 / reported_display_ratio
        return reported_display_ratio
...
def duration_seconds(data: dict, stream: dict | None, label: str) -> float:
    ...
    raise RuntimeError(f"Could not determine {label} duration")
...
def audio_info_from_stream(stream: dict) -> AudioInfo:
    ...
    if sample_rate:
        rate_label = f"{sample_rate // 1000}kHz" if sample_rate % 1000 == 0 else f"{sample_rate}Hz"
    else:
        rate_label = "Unknown Rate"

    return AudioInfo(
        profile=f"{channel_label} {rate_label}",
        channels=channels,
...
) -> tuple[list[SourceItem], list[SkippedMedia]]:
...
def format_stem_collision_report(collisions: list[StemCollision]) -> str:
    lines = [
        "Resolve-unsafe same-stem media collision detected.",
        "",
        "DaVinci Resolve can crash during OTIO import when different media files",
        "in the same folder share the same filename stem, for example IMG_0001.JPG",
        "and IMG_0001.MOV. Rename one side of each collision, then generate again.",
        "",
        "Detected collisions:",
    ]
    ...
    for index, collision in enumerate(collisions, start=1):
        lines.append(f"")
        lines.append(f"{index}. Stem: {collision.items[0].path.stem}")
        for item in collision.items:
            lines.append(f"   Current:  {item.path}")
            lines.append(f"   Proposed: {proposed_collision_path(item)}")

    lines.extend(
        [
            "",
            "No files were renamed and no OTIO file was written.",
            "After reviewing the proposals, rename the source media files yourself",
            "or rerun with --allow-stem-collisions to force OTIO generation.",
        ]
    )
    return "\n".join(lines)
...
def track_kind_for_clip(clip: TimelineClip) -> otio.schema.TrackKind:
    if clip.role == "visual":
...
def build_timeline(
    ...
    resolve_link_start_id: int,
    progress: ProgressReporter,
) -> tuple[otio.schema.Timeline, list[TimelineClip]]:
    clips = build_timeline_clips(
        source_items,
        audio_mode,
        resolve_layout,
        resolve_link_start_id,
    )
    if not clips:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate a Resolve-oriented OTIO timeline from folders of images, "
    ...
    parser.add_argument(
        ...
        default="linked",
        help=(
            "Audio handling. linked creates Resolve-linked V/A pairs for videos "
            "with embedded audio. Default: linked."
        ),
    )
    parser.add_argument(
        "--resolve-layout",
        ...
        default="organized",
        help=(
            "Resolve track compatibility layout. organized keeps audio grouped "
            "by audio profile; linked-pairs groups linked embedded audio by the "
            "matching video folder/aspect track. Default: organized."
        ),
    )
    parser.add_argument(
        ...
        default=2,
        help=(
            "First Resolve_OTIO Link Group ID used for linked video/audio pairs. "
            "Default: 2, matching the tested Resolve finding."
        ),
    )
    parser.add_argument(
        "--file-url-mode",
        ...
        default="resolve",
        help=(
            "resolve keeps spaces unescaped for DaVinci Resolve; encoded uses "
            "standard percent-encoded file URIs. Default: resolve."
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Generate even when files in the same folder share the same stem "
            "with different extensions. Not recommended for DaVinci Resolve."
        ),
    )
...
def main() -> None:
    ...
    source_items, skipped = build_source_items(
    ...
    stem_collisions = find_stem_collisions(source_items)
    if stem_collisions and not args.allow_stem_collisions:
        print(format_stem_collision_report(stem_collisions), file=sys.stderr)
        sys.exit(2)

    timeline, clips = build_timeline(
    ...
    visual_clips = sum(1 for clip in clips if clip.role == "visual")
    linked_audio_clips = sum(1 for clip in clips if clip.role == "linked_audio")
    standalone_audio_clips = sum(1 for clip in clips if clip.role == "standalone_audio")

    print(f"Generated: {output}")
    print(f"Timeline: {args.timeline_name}")
    print(f"Source items: {len(source_items)}")
    print(f"Visual clips: {visual_clips}")
    print(f"Linked audio clips: {linked_audio_clips}")
    print(f"Standalone audio clips: {standalone_audio_clips}")
    print(f"Tracks: {len(timeline.tracks)}")
    print(f"FPS: {args.fps:g}")
    print(f"Image duration: {args.image_duration:g}s")
